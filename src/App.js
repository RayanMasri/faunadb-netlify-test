	...
	clearCompleted = () => {
		...
		this.setState(
			...
			() => {
				...
				api.batchDelete(data.completedTodoIds)
					.then(() => {
			...
			return (
				<div key={i} className='todo-item'>
					<label className='todo'>
						<input data-id={id} className='todo__state' type='checkbox' onChange={this.handleTodoCheckbox} checked={data.completed} />
			...
			<div className='app'>
				...
				<div className='todo-list'>
					...
					<form className='todo-create-wrapper' onSubmit={this.saveTodo}>
						<input className='todo-create-input' placeholder='Add a todo item' name='name' ref={(el) => (this.inputElement = el)} autoComplete='off' style={{ marginRight: 20 }} />
				...
				<SettingsMenu showMenu={this.state.showMenu} handleModalClose={this.closeModal} handleClearCompleted={this.clearCompleted} />
			</div>
		);
	}
}
